...
/**
 * Dispatches an urgent WhatsApp/SMS alert to the on-call nurse when a patient escalation occurs.
...
export async function notifyNurseSMS(
  ...
  if (
    ...
    rawFrom === 'your_twilio_whatsapp_from_here' ||
    rawFrom === 'your_twilio_phone_number_here' ||
    to === 'your_nurse_phone_number_here'
  ) {
    console.warn(
      '[notifyNurseSMS] Twilio credentials or WhatsApp/phone numbers missing/placeholder. Skipping live WhatsApp dispatch.'
    );
    return;
  }
