...
if (accountSid && authToken && accountSid !== 'your_twilio_account_sid_here') {
  try {
    ...
    console.warn('[notifyNurseSMS] Failed initializing Twilio client at module load:', err);
  }
}
...
export async function notifyNurseSMS(
  ...
  if (!twilioClient || !from || !to || from === 'your_twilio_phone_number_here' || to === 'your_nurse_phone_number_here') {
    console.warn(
  ...
  try {
    ...
    console.log(`[notifyNurseSMS] SMS dispatch successful. Message SID: ${message.sid}`);
  } catch (err) {
